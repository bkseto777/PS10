...
```{r train lm}
m1 <- lm(model_formula, data=train)
y_hat_lm_train <- predict(m1, newdata=train)
train_lm <- sqrt(mean(train$log_price_doc - y_hat_lm_train)^2)
```
...
y_hat_lasso_train <- predict(LASSO_fit, newx=predictor_matrix_train, s=lambda_star_1SE) %>% 
  as.vector()

train_lasso <- sqrt(mean(train$log_price_doc - y_hat_lasso_train)^2)
```
...
```{r lm}
y_hat_lm <- predict(m1, newdata=test)
```
...
```{r test}
sample_submission$price_doc <- 2.72^y_hat_lm
write_csv(sample_submission, "lm.csv")

sample_submission$price_doc <- 2.72^y_hat_lasso
write_csv(sample_submission, "lasso.csv")
```
...
--------------------- | -------------   | -------------
lm                    | 1.82e-10        |  2.63673
LASSO crossvalidated  | 2.27e-15        |  0.44949

Note that while the lasso model was superior to the giant lm, the spline model was better than both (see screenshot).  Interesting.
