...
I used only 150 training observations to train the model, rather than all of the training data.  But, I used Lasso to pick the best predictor, rather than whatever I used before.  
...
LASSO_fit <- glmnet(x=predictor_matrix_train, y=train$price_doc, alpha = 1, lambda = lambda_inputs)


LASSO_coefficients <- get_LASSO_coefficients(LASSO_fit)

lasso <- LASSO_coefficients %>% filter(estimate != 0, term != "(Intercept)")

best <- lasso %>% mutate(best = ifelse(lambda == max(lasso$lambda),1,0)) %>% filter(best == 1)

best
```

Turns out full_sq is still my best predictor.  Was it all for nothing?

```{r}
model_spline <- with(train ,smooth.spline(x=full_sq, y=price_doc, cv=TRUE))

spline_fitted <- predict(model_spline, x=test$full_sq)

spline_fitted <- spline_fitted %>% 
  as_tibble() %>% 
  rename(hp = x, .fitted = y)

submission <- data.frame(id = test$id,price_doc=spline_fitted$.fitted)
#write_csv(submission, "try2.csv")
```
...
Our scores are not too different. 
